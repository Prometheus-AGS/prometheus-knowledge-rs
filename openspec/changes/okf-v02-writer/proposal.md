...
- `sources` is written as a list of mappings, `{ id, resource, … }`. **BREAKING**, in three places, and the first draft of this proposal named only the first:
  ...
  - **mixed pk versions on one knowledge base** (observed, see `evidence.md`): pk 1.8.0 silently skips every entry 1.9.0 writes (`skipping malformed entry`) and fails every snapshot 1.9.0 commits (`invalid type: map, expected a string`), so its retrieval goes empty. Inherent in v0.2's mapping-form `sources`; not fixable from this side. **Every pk binary that reads a knowledge base must be upgraded together.**
